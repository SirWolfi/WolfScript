...
std::vector<std::string> IniHelper::tls::split_by(std::string str,std::vector<char> erase, std::vector<char> keep, std::vector<char> extract, bool ignore_in_braces, bool delete_empty, bool lookfor_string) {
    std::vector<std::string> ret;
    std::string tmp;
    int ignore = 0;
    bool in_str = false;
    ...
    for(size_t i = 0; i < str.size(); ++i) {
        ...
        if(isIn(str[i],{'"'}) && lookfor_string && ignore == 0) {
            in_str = !in_str;
            just_found_str = true;
        }

        if(isIn(str[i],erase) && ignore == 0 && !in_str) {
            ret.push_back(tmp);
            tmp = "";
            continue;
        }
        if(isIn(str[i],keep) && ignore == 0 && !in_str) {
            ret.push_back(tmp + str[i]);
            tmp = "";
            continue;
        }
        if(isIn(str[i],extract) && ignore == 0 && !in_str) {
            ret.push_back(tmp);
